...
Does nothing.  Evisioned as a means to run ModelTest individualy against a huge number of phenotypes.
...
sub _test {
	...
	# execute the paup file
	`win-paup4b10.exe $params{'-paupfile'}\n`;
	# execute the mt file
}
...
sub process {
	# writes a modeltest file to be used in PAUP*
	# MODELTEST file copyright ()
	my %raw_params = @_;
	my %default_params = ('-path' => '', '-outfile' => 'mt_out.nex');
	my %params = (%default_params, %raw_params);
	
	open (OUT, ">$params{'-path'}$params{'-outfile'}") or die "can't open outfile";
	print OUT <DATA>;
	close OUT;
}
